...
# **HTTP/websocket服务器[(参考地址)](https://wiki.swoole.com/#/websocket_server)**
...
HTTP 服务器的路由定义文件位于`./confg/http.php`中。声明具体的路由规则，第一个参数为定义的浏览器访问地址，第二个参数`@`前半部分为文件的完整命名空间，`@`后半部分为在类中调用的具体方法。
...
websocket服务器的路由定义文件位于`./confg/websocket.php`中。
```
return [
